...
const defaultHeroImage = "/templates/doctor-2.1/medical-practice-hero.webp";
...
const itemInitials = (item: Readonly<Record<string, JsonValue>>, key: string): string => {
        ...
        <div className="heroInner">
          ...
          <div className="heroVisual medivraVisual">
            <img
              alt={localeText(
                context.locale,
                "A welcoming doctor in a modern private clinic",
                "طبيب ودود داخل عيادة خاصة حديثة",
              )}
              src={
                field(value, "heroMediaId")
                  ? context.media.url(field(value, "heroMediaId"))
                  : defaultHeroImage
              }
            />
            <div className="medivraVisualBadge">
              <strong>4.9/5</strong>
              <span>{localeText(context.locale, "Patient experience", "تجربة المرضى")}</span>
            </div>
          </div>
        </div>
      </section>
            ...
            <article className="infoCard" key={typeof item.id === "string" ? item.id : index}>
              {typeof item.imageMediaId === "string" && item.imageMediaId ? (
                ...
                <div aria-hidden className="infoCardMedia infoCardMedia--placeholder">
                  <span>{itemInitials(item, "name") || String(index + 1).padStart(2, "0")}</span>
                </div>
              )}
        ...
        <div className="contactLayout">
          <div className="contactGrid">
            <a className="contactCard" href={`tel:${field(value, "phone").replace(/\s/g, "")}`}>
              <small>{localeText(context.locale, "Call the practice", "اتصل بالعيادة")}</small>
              <strong>{field(value, "phone")}</strong>
            </a>
            <a className="contactCard" href={`mailto:${field(value, "email")}`}>
              <small>{localeText(context.locale, "Send an email", "أرسل بريدًا إلكترونيًا")}</small>
              <strong>{field(value, "email")}</strong>
            </a>
            <div className="contactCard">
              <small>{localeText(context.locale, "Visit us", "زورنا")}</small>
              <strong>{field(value, "address")}</strong>
            </div>
            <div className="contactCard">
              <small>{localeText(context.locale, "Opening hours", "ساعات العمل")}</small>
          ...
          <div className="doctorMapPanel">
            ...
            >
              ...
              <span>
                <small>{localeText(context.locale, "Find the practice", "موقع العيادة")}</small>
                <strong>{field(value, "address")}</strong>
                <em>{localeText(context.locale, "Open full map →", "افتح الخريطة كاملة ←")}</em>
              </span>
            </a>
          </div>
        </div>
