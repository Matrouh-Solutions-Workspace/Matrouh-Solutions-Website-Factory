...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (typeof body.eventType !== "string" || !allowedEvents.has(body.eventType)) return Response.json({ error: "INVALID_EVENT" }, { status: 400 });
    ...
    const sessionHash = createHash("sha256").update(`${clientAddress(request)}:${request.headers.get("user-agent") ?? ""}`).digest("hex");
    await withTenantTransaction(
      client,
      { organizationId: store.organization_id, actorId: "00000000-0000-0000-0000-000000000000", correlationId: `storefront-event:${randomUUID()}` },
      (transaction) => transaction.ecommerceAnalyticsEvent.create({
        data: {
          id: randomUUID(), organizationId: store.organization_id, storeId: store.store_id,
          eventType: body.eventType as string, sessionHash, dataJson: productId ? { productId } : {},
        },
      }),
    );
