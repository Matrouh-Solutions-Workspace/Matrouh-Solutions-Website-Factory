...
import { WebsiteInventory, WebsiteInventoryGroup, WebsiteInventoryItem, type InventoryEntry } from "@/app/website-inventory";
...
export default async function WebsitesPage({
  ...
  const status = ["all", "live", "draft", "paused", "unpublished", "disabled"].includes(filters.status ?? "")
    ? filters.status!
    : filters.status === "published" || filters.status === "active" ? "live" : "all";
  const websites = overview.websites.filter((website) => !website.templateId.startsWith("ecommerce:"));
  ...
  return (
    <>
      ...
      <header>
        ...
        <a className="buttonLink" href="#create-workspace">Create a website or store</a>
      </header>
      ...
      <section className="workspaceGrid websitesWorkspace">
        <div className="panel websiteInventoryPanel" id="website-inventory">
          ...
          <WebsiteInventory categories={categories} entries={inventoryEntries} initialQuery={filters.q ?? ""} initialStatus={status} initialType={type} locale={locale}>
          {websiteGroups.map((group) => (
            <WebsiteInventoryGroup entries={group.websites.map((website) => inventoryById.get(website.id)!)} key={group.category} label="Template websites" title={group.category}>
          ...
          {stores.length > 0 ? (
            <WebsiteInventoryGroup entries={stores.map((store) => inventoryById.get(store.id)!)} label="Commerce system" title="E-commerce stores">
                    ...
                    <div className="unifiedCommerceRow" key={store.id}>
                      ...
                      <div className="websiteIdentity">
                        ...
                        <small className="unifiedStoreOwner">
                          {store.owner ? `Owner: ${store.owner.displayName}` : "Owner not claimed"}
                        </small>
                        {hostname ? (
                          <a
                            className="websiteDomain"
        ...
        <div className="panel publishJobsPanel" id="publish-jobs">
          ...
          <PaginatedPublishJobs locale={locale} items={overview.publishJobs.map((job) => (
        ...
        <WebsiteCreationSwitcher initialMode={createCommerce ? "commerce" : "website"} commerceForm={
          <CommerceCreatePanel
